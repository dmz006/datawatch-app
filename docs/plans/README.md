...
| BL30 | Android Auto: full PRD/story/task view + lifecycle parity | **Priority 2 — after BL29 phone work is solid.** Auto today has session list, monitor, PRD detail (v1.3.0), clickable stories list + story detail with Approve/Reset-Task (v1.4.0). Gaps remaining: (a) task-level view — a dedicated task detail screen or inline task expansion in story detail showing full task output, error detail, verification result, and retry/skip options; (b) PRD edit from Auto — ability to edit spec or story description before approving/running; (c) deeper lifecycle parity with phone — cancel individual story, requeue failed task beyond just the first one, view task execution log/output; (d) navigation polish — currently at the 5-screen Car App depth limit (Home→Automata→PRDDetail→Stories→StoryDetail); consider flattening to free up a level for task detail; (e) Auto-specific UX: progress indicators, estimated completion, last-run timestamps readable at a glance. All new screens must use `MessageTemplate` (TTS-readable) or `ListTemplate`; no custom views. Gated on BL29 phone work landing first so the transport + DTO layer is stable. |
...
| BL32 | GPU display info parity — observer card, session stats, PRD session resources | Tracked by [datawatch-app#173](https://github.com/dmz006/datawatch-app/issues/173). Three surfaces need to match the BL380 PRD resource card (reference impl in v8.26.0): (1) **Observer card** — add temperature (°C) + power draw (W) to GPU rows alongside util%; (2) **Session stats tab** — when session has `compute_node_ref`, fetch `/api/compute/nodes/{ref}/detail` for the GPU section instead of `/api/stats` (shows remote-node GPU, not local); (3) All three surfaces use the same locale keys: `obs_cn_gpu_util`, `obs_cn_gpu_vram`, `obs_cn_gpu_temp`, `obs_cn_gpu_power`. Multi-GPU nodes label as "GPU 1 util / GPU 2 util" consistently. No server changes required — all needed endpoints already exist. |
| BL33 | Android Auto: per-guardrail approval buttons on Block Details screen | Tracked by [datawatch-app#150](https://github.com/dmz006/datawatch-app/issues/150). Currently `runSessionGuardrail(sessionId)` approves all blocks at once; needs per-verdict approval. **Blocked on server endpoint**: `runSessionGuardrail(sessionId, guardrailId)` must exist in datawatch before app work begins. Once server signals ready: update `AutoSessionDetailScreen` and Block Details screen to render per-verdict approve actions (ListTemplate for multiple blocks, since Car App Library's MessageTemplate caps at 2 buttons). File server issue when picking this up. |
| BL34 | Google Assistant App Actions ("Ok Google") — voice commands to sessions | Tracked by [datawatch-app#148](https://github.com/dmz006/datawatch-app/issues/148). Register a `SEND_MESSAGE` BII capability via `shortcuts.xml` so "Ok Google, send [command] to datawatch" / "Ok Google, tell datawatch [command]" injects input into the last-active session via `POST /api/sessions/{id}/input`. Requires: (a) `shortcuts.xml` with `actions.intent.SEND_MESSAGE` capability + `Intent` mapping, (b) deep-link or intent receiver in `MainActivity` to extract the message parameter and call the transport, (c) fallback: if no active session, speak an error. Complements BL31 Auto voice but targets ambient phone/home-speaker usage rather than in-car. |
