...
|----|-------|----------|--------|-------|
| B1 | Terminal freezes on session open; no scroll, no font adjust, no ANSI colour render | 2026-04-19 | Open | Reproduced against session `787e*` on user's device. Symptoms together (no scroll + no font + no colour) point at either WebView state leak across session switches (`TerminalView` holds `lastWrittenIndex`/`ready` in `remember {}` unkeyed to `sessionId`), a FitAddon sizing race (viewport captured at 0×0 → tiny row count), or `onReady` never firing so the initial flush is skipped. Adjustable terminal dimensions is a separate ask tracked as BL13. Plan: [2026-04-19-terminal-hardening.md](2026-04-19-terminal-hardening.md). |
