            ...
            onSend = {
                ...
                scope.launch {
                    try {
                        ...
                        val profile = (
                            enabled.firstOrNull { it.id == activeId }
                                ?: enabled.firstOrNull()
                        )
                        if (profile != null) {
...
private const val DOCS_STYLE_TAG =
    "<style id=\"dw-mobile-docs-style\">" +
...
private fun injectDocsStyleOverride(
    rawBody: ByteArray,
    charset: String,
): ByteArray {
    val charsetObj =
        runCatching { java.nio.charset.Charset.forName(charset) }
