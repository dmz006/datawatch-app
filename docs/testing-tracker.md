...
| Shared | `RestTransport` PRD/channel/backend CRUD | Yes | No | FF | androidUnitTest + MockWebServer | `RestTransportAutonomousTest` — 25 tests: listPrds, createPrd (dir+profile), prdAction (approve/reject/decompose/set_llm), editStory, editFiles, patchPrd, deletePrd (soft+hard), listBackends (obj+string shapes, filters enabled=false+shell), listChannels (wrapped+bare), createChannel, deleteChannel, setChannelEnabled, listOllamaModels, listOpenWebUiModels |
