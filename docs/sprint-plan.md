...
| v0.36.0 | Federated monitoring suite | Four new cards on Settings → Monitor: Federated peers (with All/Standalone/Cluster/Agents filter pills), Cluster nodes (Shape C with pressure flags + CPU/Mem bars), eBPF status (3-flag block + message), Plugins (subprocess + native rows with kind badges). Closes [#2](https://github.com/dmz006/datawatch-app/issues/2)–[#6](https://github.com/dmz006/datawatch-app/issues/6); [#7](https://github.com/dmz006/datawatch-app/issues/7) deferred to v0.36.1. New `/api/observer/{stats,peers}` and `/api/plugins` endpoints on `TransportClient`. |
