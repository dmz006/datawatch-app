...
| B63 | ✅ Stats: per-process eBPF network viewer | Done (v0.57.0) — `EBpfNetworkCard` in Settings → Monitor, after `EBpfStatusCard`. Reads `StatsDto.envelopes`, filters rows where `netRxBps + netTxBps > 0`, sorts by total bandwidth. Only renders when `ebpfActive == true`. Shows per-process label, root PID, RX, TX columns. |
