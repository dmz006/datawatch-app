...
| BL15 | Localization (DE, ES, FR, JA) | Done (v0.51.0) — Full string-resource extraction across all UI screens (SettingsScreen, StatsScreen, AlertsScreen, AutonomousScreen, SessionsScreen, SessionDetailScreen, NewSessionScreen, NewPrdDialog, PrdDetailDialog, BottomNavBar). EN base + DE/ES/FR/JA translations. `SettingsTab` enum migrated to `@StringRes Int` labels; SecurityCard migration error uses format string. |
