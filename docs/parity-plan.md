...
**Context.** v1.0.0 shipped as "first production release" but the PWA
(`internal/server/web/` in the parent repo) exposes a broader feature
surface than the mobile app currently wires. This document is the honest
accounting of the gap, grouped by PWA screen, plus a concrete sprint
plan to close it.
...
| Session rename (`/api/sessions/rename`) | ❌ | v1.1 |
| Session restart (`/api/sessions/restart`) | ❌ | v1.1 |
| Bulk delete completed (`/api/sessions/delete`) | ❌ | v1.1 |
| Per-row timeline view (`/api/sessions/timeline`) | ❌ | v1.2 |
| Sort by last activity / start time | ❌ | v1.1 |
| Pagination / "load more" | ❌ | v1.2 |
| Schedule: list pending for a session (`/api/schedules`) | ❌ | v1.2 |

## 2. Session detail

| PWA capability | Mobile | Notes |
|---|---|---|
| xterm.js terminal as primary view | ✅ | Shipped in v1.0.1 (terminal swap) |
...
| Mute per-session | ✅ | |
| Rename session | ❌ | v1.1 |
| Restart session | ❌ | v1.1 |
| Delete session | ❌ | v1.1 |
| Terminal copy action | ❌ | xterm selection works but no clipboard button |
| Terminal search (`xterm-addon-search`) | ❌ | v1.1 |
| Inline schedule actions (create scheduled reply) | ❌ | v1.2 |
| Backlog pager (`/api/sessions/timeline`) | ❌ | v1.2 |
...
| PWA capability | Mobile | Notes |
|---|---|---|
| Start session from form (`/api/sessions/start`) | ❌ | v1.1 — transport has `startSession`, UI missing |
| Pick LLM backend (`/api/backends`) | ✅ (read-only) | Channels tab; per-session backend selection during start is v1.1 |
| Pick model variant | ❌ | v1.1 — needs `/api/ollama/models` + `/api/openwebui/models` |
| Pick profile (`/api/profiles`) | ❌ | v1.1 — F10 ephemeral-agent profiles |
| Directory picker (`/api/files`) | ❌ | v1.2 |
| Voice-to-new-session | ❌ | v1.2 — composer mic exists; "new:" prefix auto-exec wiring needed |
...
| Mark-as-read / dismiss | ❌ | v1.1 — swipe-to-mute exists; explicit dismiss missing |
| `/api/alerts` read | ❌ | v1.1 — mobile currently derives from session list; PWA reads a dedicated endpoint for richer metadata |
| Schedule actions from alerts | ❌ | v1.2 |
...
| Federated server list (read-only) | ❌ | v1.2 — shows peers of a server |

### 5b. LLM backend config

| PWA | Mobile | Notes |
|---|---|---|
| Pick active backend | ❌ | v1.1 — `POST /api/backends/active` (parent to confirm shape) |
| Edit endpoint URL / API key per backend | ❌ | v1.2 — structured form per ADR-0019 |
| Pick Ollama model (`/api/ollama/models`) | ❌ | v1.2 |
| Pick OpenWebUI model (`/api/openwebui/models`) | ❌ | v1.2 |

### 5c. Channels (messaging backends)

| PWA | Mobile | Notes |
|---|---|---|
| List configured channels | ❌ | v1.2 — awaits parent `/api/channels` exposure |
| Add / remove channel | ❌ | v1.2 |
| Test message round-trip (`/api/channel/send`) | ❌ | v1.2 |
| Per-channel enable / disable | ❌ | v1.2 |
| Download CA cert (`/api/cert`) | ❌ | v1.1 — drives self-signed TLS trust |

### 5d. Daemon control + introspection

| PWA | Mobile | Notes |
|---|---|---|
| Show daemon version (`/api/health`) | ❌ | v1.1 — trivial; add to About card |
| Connection status indicator | ❌ | v1.1 — transport already has `isReachable` Flow |
| Config read (`GET /api/config`) | ❌ | v1.2 — read-only view |
| Config write (`PUT /api/config`) | ❌ | v1.3 — guarded per ADR-0019 |
| Recent logs (`/api/logs`) | ❌ | v1.2 — streaming viewer |
| Network interfaces (`/api/interfaces`) | ❌ | v1.2 |
| Restart daemon (`/api/restart`) | ❌ | v1.2 — confirm dialog |
| Update daemon (`/api/update`) | ❌ | v1.3 |

### 5e. Session preferences

| PWA | Mobile | Notes |
|---|---|---|
| Input mode (tmux / channel / none) | ❌ | v1.2 |
| Output mode | ❌ | v1.2 |
| Recent-session retention window | ❌ | v1.2 |
| Max concurrent sessions | ❌ | v1.2 |
| Scrollback line count | ❌ | v1.2 |
...
| Active-session suppression | ❌ | v1.1 — don't fire for sessions in foreground |
...
| PWA | Mobile | Notes |
|---|---|---|
| Memory stats (`/api/memory/stats`) | ❌ | v1.3 |
| List memories (`/api/memory/list`) | ❌ | v1.3 |
| Search memories (`/api/memory/search`) | ❌ | v1.3 |
| Delete memory (`/api/memory/delete`) | ❌ | v1.3 |
| Export memory (`/api/memory/export`) | ❌ | v1.3 |

### 5h. Schedules

| PWA | Mobile | Notes |
|---|---|---|
| List all schedules (`/api/schedules`) | ❌ | v1.2 |
| Create scheduled reply for a session | ❌ | v1.2 |
| Cancel schedule | ❌ | v1.2 |
...
| Uptime | ✅ | |
| Per-process eBPF network | ❌ (view-only) | v1.3 — post-MVP per ADR-0019 |
| Session counts | ✅ | |
...
### v1.1.0 — Session power-user parity
...
### v1.2.0 — Channels + schedules + file picker
...
### v1.3.0 — Memory / KG + daemon config
