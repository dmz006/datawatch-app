...
class AutonomousViewModelTest {
    ...
    @Test
    fun `refresh populates prds on success`() = runTest(testDispatcher) {
    ...
    @Test
    fun `refresh sets banner on transport failure`() = runTest(testDispatcher) {
        val (t, r) = fakeResolver()
    ...
    @Test
    fun `refresh shows no-server banner when resolver returns null`() = runTest(testDispatcher) {
    ...
    @Test
    fun `create posts the request body and triggers a refresh`() = runTest(testDispatcher) {
        val (t, r) = fakeResolver()
    ...
    @Test
    fun `approve calls prdAction approve and refreshes`() = runTest(testDispatcher) {
        val (t, r) = fakeResolver()
    ...
    @Test
    fun `reject sends the reason in the body`() = runTest(testDispatcher) {
        val (t, r) = fakeResolver()
        coEvery { t.prdAction(any(), any(), any()) } returns Result.success(Unit)
        coEvery { t.listPrds() } returns Result.success(PrdListDto())
    ...
    @Test
    fun `editStory passes only blank-stripped fields through`() = runTest(testDispatcher) {
        val (t, r) = fakeResolver()
    ...
    @Test
    fun `editFiles forwards the files list`() = runTest(testDispatcher) {
        val (t, r) = fakeResolver()
