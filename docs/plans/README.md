...
| B10 | Monitor — no live system-stats streaming | 2026-04-22 | Deferred | StatsScreenContent polls `/api/stats` every 5 s. Server does broadcast `MsgStats` WS frames (`internal/server/ws.go:42`) but EventMapper doesn't surface them yet. Wiring a live StatsFlow through the WebSocketTransport + consuming it in StatsScreenContent is a v0.34 candidate. Current 5 s polling is functional. |
| B11 | Monitor — no session-stats panel with eBPF wheels/graphs | 2026-04-22 | Deferred | Extends B5. PWA has per-session CPU / mem / net wheel + per-backend graphs. Full scope post-1.0 (ADR-0019). |
...
| B27 | Terminal blank / no live update after first pane_capture | 2026-04-22 | Investigating | Logcat confirms WS reconnects every 3-5 s even after v0.33.12 Data Layer + v0.33.13 watchdog-removal fixes. Each reconnect fires the server's initial CapturePaneANSI (huge first-tick burst) but the 200ms StartScreenCapture ticker gets killed by the disconnect before any incremental updates flow. v0.33.15 added verbose cause-chain logging to the WS catch block so next on-device trace reveals whether it's OkHttp ping timeout, server-side idle close, or tailnet path flap. Also added scrollToBottom after pane_capture write so a CapturePaneANSI-sized first frame doesn't leave the viewport stranded mid-scrollback. |
