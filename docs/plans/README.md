...
| BL33 | Android Auto: per-guardrail approval buttons on Block Details screen | Tracked by [datawatch-app#150](https://github.com/dmz006/datawatch-app/issues/150). Currently `runSessionGuardrail(sessionId)` approves all blocks at once; needs per-verdict approval. **Blocked on server endpoint**: `POST /api/sessions/{id}/guardrail/{name}/approve` filed as [datawatch#153](https://github.com/dmz006/datawatch/issues/153). Once server ships: add `approveGuardrailBlock(sessionId, guardrailName)` to `TransportClient`, update Auto Block Details screen with per-verdict approve buttons in a `ListTemplate` (Car App Library `MessageTemplate` caps at 2 buttons). |
