...
| BL29 | PRD full lifecycle management — phone first, then Auto + iOS | **Priority 1 — do before BL30.** Server API audit complete (2026-09-13). **Already wired end-to-end:** patchPrd (edit title/spec), editStory (title/description), editFiles, approve/reject/request_revision/cancel/run/decompose via prdAction, resetPrdTask (failed/blocked tasks), AutonomousViewModel with reject(reason) + requestRevision(note). **Server gaps — issues filed on dmz006/datawatch:** cancel_story [#149], cancel_task [#150], requeue completed task [#151]; umbrella issue with PWA + MCP + API docs + comms events + federation requirements [#152]. **App-only gaps — issues filed on dmz006/datawatch-app:** editTask() transport method + UI [#171], approve missing note field in UI [#172]. **Remaining implementation work (no server changes needed):** (a) wire editTask #171 into PrdDetailDialog task rows; (b) add note field to approve dialog (#172); (c) surface cancel_story/cancel_task/requeue actions in PrdDetailDialog once server endpoints land (#149 #150 #151); (d) Auto + iOS parity once phone is solid (see BL30). |
