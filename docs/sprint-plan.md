...
| v0.35.0–v0.35.5 | Final PWA-parity milestone + Wear feature arc | LLM backend config paths fixed, inline rename, state-override dropdown, Response composer button, Sessions list UX (search collapse + right-side reachability dot), Wear round bezel cards, release workflow fix, R8 `-dontwarn` hardening, Wear Monitor colour gauges (CPU/Mem/Disk/GPU with thresholds), Wear Sessions tap-popup with voice-to-text reply via transient WS. **Parity milestone closed 2026-04-24 at v0.35.2**; v0.35.3–v0.35.5 ship post-milestone UX + Wear feature polish per [`docs/plans/audit-2026-04-23/README.md`](plans/audit-2026-04-23/README.md). |
