...
| B64 | Docs: README screenshots, Android section, Android Auto demo GIF | Done (v0.46.0) — phone+watch slideshows, Android/Wear/Auto sections added. Auto GIF pending AAOS emulator screenshots. |
| B65 | PRDs tab: rename to "Autonomous", match PWA icon + header name | PWA calls the tab "Autonomous" with a matching icon; mobile still shows "PRDs". Rename tab label + icon + screen title to align. |
| B66 | PRDs/Autonomous page: card-style layout matching Sessions page | Sessions list uses card-per-session with badge, state pill, quick-actions. PRDs page uses a different layout. Match sessions card style + add datawatch eye watermark in background (like Sessions page). Move FAB to same position as sessions FAB. Upstream: [dmz006/datawatch#30](https://github.com/dmz006/datawatch/issues/30). |
| B67 | AAOS: app crashes on launch — AppWidgetManager null on Automotive | `SessionsWidget.requestUpdate` crashes with NPE on AAOS because `AppWidgetManager.getInstance()` returns null (no home-screen widgets on Automotive OS). Fixed: null-check guard added in `SessionsWidget.kt:155`. |
| B68 | Android Auto: app icon eye overflows icon boundary | The datawatch eye graphic extends past the edges of the launcher icon on AAOS app grid. Should fit within the rounded-square icon boundary. Fix: shrink/pad the foreground layer in the adaptive icon XML. |
| B69 | Android Auto: add read-only Autonomous/PRDs tab | Auto surface currently has Sessions, Alerts, Settings. PRDs/Autonomous plans are not shown. Add a read-only Autonomous tab so users can view active PRDs from the car display. |
