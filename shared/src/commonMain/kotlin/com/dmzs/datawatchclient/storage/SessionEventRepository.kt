...
public class SessionEventRepository(
    ...
    public fun observe(sessionId: String, limit: Long = RETAIN_PER_SESSION): Flow<List<SessionEvent>> =
    ...
    public suspend fun insert(event: SessionEvent): Unit = withContext(ioDispatcher) {
        when (event) {
            ...
            is SessionEvent.StateChange -> db.eventQueries.insertEvent(
                session_id = event.sessionId,
                ts = event.ts.toEpochMilliseconds(),
                type = "state",
                body = null,
            ...
            is SessionEvent.PromptDetected -> db.eventQueries.insertEvent(
                session_id = event.sessionId,
                ts = event.ts.toEpochMilliseconds(),
                type = "prompt",
                body = null,
            ...
            is SessionEvent.RateLimited -> db.eventQueries.insertEvent(
                session_id = event.sessionId,
                ts = event.ts.toEpochMilliseconds(),
                type = "rate",
                body = null,
            ...
            is SessionEvent.Completed -> db.eventQueries.insertEvent(
                session_id = event.sessionId,
                ts = event.ts.toEpochMilliseconds(),
                type = "done",
                body = null,
            ...
            is SessionEvent.Error -> db.eventQueries.insertEvent(
                session_id = event.sessionId,
                ts = event.ts.toEpochMilliseconds(),
                type = "error",
                body = null,
            ...
            is SessionEvent.Unknown -> db.eventQueries.insertEvent(
                session_id = event.sessionId,
                ts = event.ts.toEpochMilliseconds(),
                type = event.type,
                body = null,
