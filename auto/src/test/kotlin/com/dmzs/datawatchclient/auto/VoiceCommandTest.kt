...
class VoiceCommandTest {
    ...
    @Test fun `report phrase maps to REPORT`() =
        assertEquals(
            VoiceCommand.REPORT,
            parseVoiceCommand("give me a summary"),
        )
    ...
    @Test fun `automata status phrase`() =
        assertEquals(
            VoiceCommand.LIST_AUTOMATA,
            parseVoiceCommand("automata status"),
        )
    ...
    @Test fun `memory recall phrase`() =
        assertEquals(
            VoiceCommand.MEMORY_RECALL,
            parseVoiceCommand("recall the auth module"),
        )
