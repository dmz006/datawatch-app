...
| ID | Title | Notes |
|----|-------|-------|
| B31 | ✅ Wear session snapshot — `lastLine` shown as third subtitle line on each session row (current activity preview without opening popup). Quick-command (Continue/Stop) + voice reply already shipped in popup (v0.101.0). Full sessions list with state groups shipped Sprint 32. | v1.12.0 |
...
| BL369 | ✅ Session Stats sub-tab parity with PWA: HostCard title includes backend name; CPU/RSS sparklines inline on stat rows (`StatRowWithSparkline`); ComputeNode header includes node name, redundant StatRow removed. | v1.12.0 |
| BL368 | ✅ Session Status tab parity with PWA: TESTS/GIT placeholder cards shown when null; Sprint raw JSON always shown alongside structured rows; FocusCard shows "—" when `currentFocus` null. | v1.12.0 |
...
| BL388 | Android Auto Messaging Compliance — implement conversation-thread structure on session/PRD/story/task Auto screens; switch category from IOT → MESSAGING; resolve Play Console warnings "Message functionality" + "In-App Messaging Functionality". Conversation hierarchy: Session detail, PRD overview (play + optional spec update), Stories as sub-conversations, Tasks as discussion. ADR-0049. Plan: `docs/plans/2026-09-15-bl388-auto-messaging-compliance.md`. | backlog |
