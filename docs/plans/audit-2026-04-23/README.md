...
| G6 | **Sessions reorder: drag-drop instead of hamburger menu** | app.js:1412-1415 HTML5 drag API on `.session-card` + `sessionDragStart/Over/Drop/End` + localStorage('cs_session_order') | SessionsScreen reorder-mode toggles up/down arrows + hamburger menu entry | Replace reorder-mode + arrow buttons with Compose `reorderable` modifier (or `Modifier.pointerInput` long-press drag). Drop the hamburger menu entry. Persist `sessionOrder` to SharedPreferences to mirror PWA localStorage. | SessionsScreen.kt | P1 | v0.34.9 |
