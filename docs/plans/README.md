...
| BL27 | Wear: background battery drain + wake-on-alert | Watch app polls / holds WS connection even when not the active foreground app, draining battery. Should go fully passive (cancel polls + WS) when backgrounded. Wake on alerts: hold a `WakefulBroadcastReceiver` or `WorkManager` task that fires on incoming alert notifications so the watch can wake and display them without continuous polling. |
