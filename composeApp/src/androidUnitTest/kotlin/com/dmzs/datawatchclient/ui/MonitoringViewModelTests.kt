...
class MonitoringViewModelTests {
    ...
    @Test
    fun `federated peers loads + filters`() =
    ...
    @Test
    fun `federated peers swallows transport failure into empty list`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `cluster nodes maps stats cluster nodes`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `cluster nodes empty when stats has no cluster block`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `ebpf status pulls host ebpf block`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `ebpf null when daemon predates observer endpoint`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `plugins splits subprocess from native`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `orchestrator graph stores nodes and edges`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
            val graph =
    ...
    @Test
    fun `sweep stale stores count from response`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
    ...
    @Test
    fun `extract facts stores triples from response`() =
        runTest(testDispatcher) {
            val (t, r) = fakeResolver()
