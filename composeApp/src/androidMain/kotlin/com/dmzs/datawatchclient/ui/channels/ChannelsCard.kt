...
public fun ChannelsCard() {
    ...
    Column(
        ...
        PwaSectionTitle("Messaging channels")
        banner?.let {
            Text(
                it,
        ...
        if (channels.isEmpty() && banner == null) {
            Text(
                "No channels configured. Add one via the server's " +
                    "datawatch config UI (mobile add/remove is deferred " +
                    "pending parent API changes).",
