...
| BL24 | Settings → System: MCP channel bridge info card | Mirror `GET /api/channel/info` on a new MCP Channel row in Settings → System tab. Show: bridge kind (Go ✓ / JS ⚠), bridge path, ready badge, and collapsible stale-.mcp.json warning list. Refresh on tab focus. Tracks [datawatch-app#38](https://github.com/dmz006/datawatch-app/issues/38). |
