...
| BL369 | Session Stats sub-tab parity with PWA: (1) HostCard section header should include backend name ("BACKEND STATS — OPENCODE-DOCKER" not just "BACKEND STATS") using `session.backend`; (2) CPU/RSS/GPU util/GPU temp/ollama CPU sparklines should render inline on the same row as the current value (`~~~~ 1.7%` layout), not as separate blocks below; (3) COMPUTE NODE section header should include node name ("COMPUTE NODE — DATAWATCH") rather than showing it as a separate `StatRow` below the header. Screenshot comparison filed 2026-09-15. | backlog |
| BL368 | Session Status tab parity with PWA: (1) show TESTS / GIT placeholder cards even when `board.tests`/`board.git` are null ("no test signal yet — hook payload tests=… expected" / "no git state — hook payload git=… expected"); (2) always show Sprint raw JSON block alongside structured rows (not only as fallback when all fields blank); (3) show FocusCard with "—" placeholder when `board.currentFocus` is null/empty rather than hiding the card. Screenshot comparison filed 2026-09-15. | backlog |
| BL385 | Memory scope model — `prd-shared`/`story-shared` API contract + mobile parity (daemon v8.29.0, GH#174): tolerance pass for new `prd_id`/`story_id` fields on memory recall response; wire `POST /api/memory/scopes/save` and `/delete`; add `docs/parity-status.md` row. No new UI. Plan: `docs/plans/2026-09-15-memory-arc-bl385-bl387.md`. | backlog |
| BL386 | Memory lifecycle — PRD seed/harvest config + delete strategy + archive import (daemon v8.30.0, GH#175): `memory_seed` / `memory_harvest` toggles in PRD create/edit; "Memory strategy" picker in PRD and session delete dialogs; `memory_report` collapsible section on PRD detail. Plan: `docs/plans/2026-09-15-memory-arc-bl385-bl387.md`. | backlog |
| BL387 | PRD memory integration — `from_prds` cross-PRD seeding + memory_report render + memory stats tile (daemon v8.31–v8.33, GH#176): Phase 2 `from_prds` multi-select in PRD create/edit; Phase 3 memory stats row (`prd_shared_count`, `story_shared_count`, `session_local_count`) on PRD status card. Depends on BL385→BL386. Plan: `docs/plans/2026-09-15-memory-arc-bl385-bl387.md`. | backlog |
| BL-auto-messaging | ✅ Google Play compliance — Android Auto messaging review (version code 393): changed `CarAppService` category from `MESSAGING` → `OTHER` in `AndroidManifest.xml`. `CarAppExtender` Play/Reply notification buttons unaffected. ADR-0031 updated. Shipped v1.9.3. | v1.9.3 |
