...
| BL-auto-messaging | Google Play compliance — Android Auto "In-App Messaging Functionality" review failure (version code 393): Play store flags that (1) historical conversations do not appear in the "app space" and (2) outgoing/incoming messages don't work as expected. Car App Library `MessagingTemplate` may be required. Investigate whether our session-terminal use case is miscategorized as a messaging app or if we need to add `MessagingTemplate` / `ConversationTemplate` support. Plan: audit `AndroidManifest.xml` Auto category declaration + `CarAppService` capability declaration vs Play Auto quality guidelines. | backlog |
